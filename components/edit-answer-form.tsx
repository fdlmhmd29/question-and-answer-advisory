...
export function EditAnswerForm({ answer, onSuccess }: EditAnswerFormProps) {
  const router = useRouter();
  const [open, setOpen] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [isLoading, setIsLoading] = useState(false);
  const [technicalNote, setTechnicalNote] = useState(
    answer.technical_advisory_note || ""
  );
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="w-[95vw] sm:w-[50vw] max-w-[50vw] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>Perbarui technical advisory note</DialogDescription>
        </DialogHeader>
